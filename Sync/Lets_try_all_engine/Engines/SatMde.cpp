...
if(timeToCompute) // Time to Compute Visiblity this is caluclated evey 100 sec
{
timeToCompute = false;
...
// take the measurement time at which measurement would be computed
// this is an active input to this engine
DayTime measTime = satMDE_ai.measTime;

// Instantiate the modulator Engine inside the measurement engine
Modulator		modulator("ModulatorSection");
modulator.Engine(CodeGen::INITIALIZE);

// Visiblity is computed in Active state of the engine hence 
// for the method of the engine all that is ready to be processed
totalSat = visibleSV.size();

// for all the visible SV.	
for (int i=M_ZERO; i<totalSat; i++)
	{
	...
	// Pointer of the common satellite position databas is provided here as an active input to this engine
	// get measurement will take rx position and time, time and sat it is feed  to satellite database  to compute the position of that satid at that time
	// after the position is obtained then the satellite position and rx position is fed to sattelite data generator to get 
	// measurement like PR, elevation , range rate etc
	// this fuction returns the health of all the data generated and if something is wrong it will retun a false
	...
	// as we are differnetiating  the PR to get DR we have to take care of the 
	// first difference
	...
	// Doppler gets added to the received code and carrier
