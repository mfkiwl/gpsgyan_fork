...
engineHealthMonitor satMDE_probe;
...
protected : 
 ...
 //         void prepareIpToEngine() ;
			...
			double ComputeCorrectedRange(Xvt& satXvt , Xvt& rxXvt);
			double getIonoCorrections(double alpha[],double beta[],DayTime Tr,
                                                   Xvt &rxPos,
                                                   double elevation,
                                                   double azimuth,gpstk::IonoModel::Frequency );
												   
            void ComputePhaseAndDoppler(int sat_index,double rangeRate);												   
			//double getTropoCorrections( TropModel *pTropModel, double elevation );
			//void ComputeSvVisiblity();
			int findSvPrvRange(SatID vis_svid);
			double ComputePhaseWindUpDelay(int sat_index,DayTime time);	
